...
public class StarMenuHandler {
    private static boolean active = false;
    private static double cursorX = 0.0;
    private static double cursorY = 0.0;
    private static int selectedSegment = 0;

    // Параметры интерфейса (изменены для компактности)
    private static final int RADIUS = 30;           // расстояние от центра до центра ромба
    private static final int SIZE = 32;             // размер ромба
    private static final int MAX_RADIUS = RADIUS;    // курсор не выходит за круг
    private static final int ARROW_SIZE = 15; // размер текстуры стрелки
    private static final int STAR_MENU_SIZE = 128;             // размер ромба
    ...
    public static int getSelecetdSegment() {
    ...
    private static void updateSelectedSegment() {
        ...
        // Исправление: инвертируем Y для корректного угла (экранная система координат)
    ...
    private static void render(GuiGraphics guiGraphics, int width, int height) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (selectedSegment == i) guiGraphics.blit(STAR_MENU, texX, texY, 32, 22, SIZE, SIZE, STAR_MENU_SIZE, STAR_MENU_SIZE);
        }

        // Дебаг: красный квадратик
        if (DEBUG_CURSOR) {
    ...
    private static void renderArrow(GuiGraphics guiGraphics, int centerX, int centerY) {
        // Вычисляем угол направления курсора (в радианах)
        double angle = Math.atan2(cursorY, cursorX);
        // Если курсор в центре, оставляем угол 0 (стрелка вверх)
        if (cursorX == 0 && cursorY == 0) angle = -Math.PI / 2;

        // Устанавливаем режим смешивания как в ChargesOverlayEventHandler
        RenderSystem.enableBlend();
