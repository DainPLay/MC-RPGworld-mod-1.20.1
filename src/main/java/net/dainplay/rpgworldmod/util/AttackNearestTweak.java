			...
			if (canHaveGoal(mob)) {
				if (targetEntityLocation.toString().equals("minecraft:player")) {
					...
					if (targetEntity instanceof LivingEntity) {
						...
						RPGworldMod.LOGGER.error(String.format("Can't apply AI tweak of ID %s for entity %s. Target entity isn't valid for the tweak", getName(), getEntityLocation()));
					}
				}
			}
		} else {
			RPGworldMod.LOGGER.error(String.format("Can't apply AI tweak of ID %s for entity %s. Entity isn't valid for the tweak", getName(), getEntityLocation()));
		}
	}
			...
			if (goal instanceof NearestAttackableTargetGoal nearestAttackable) {
				if (targetEntityLocation.toString().equals("minecraft:player")) {
					if (nearestAttackable.targetType == Player.class) {
						RPGworldMod.LOGGER.error(String.format("Can't apply AI tweak of ID %s for entity %s. Entity already has given AI goal", getName(), getEntityLocation()));
						return false;
					}
				} else {
