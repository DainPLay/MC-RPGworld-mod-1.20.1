...
public class ActionRewardHandler {
	...
	public static void onUpdate(LivingEvent.LivingTickEvent event) {
		if (!(event.getEntity() instanceof ServerPlayer player) || !(player.getMainHandItem().is(ModItems.WEALD_BLADE.get()) || player.getOffhandItem().is(ModItems.WEALD_BLADE.get())) || player.getAdvancements().getOrStartProgress(player.getServer().getAdvancements().getAdvancement(WealdBladeGesturesTrigger.ID)).isDone())
			return;
	...
	public static void onItemUse(LivingEntityUseItemEvent.Start event) {
		if (!(event.getEntity() instanceof ServerPlayer player) || !event.getItem().is(ModItems.WEALD_BLADE.get()) || player.getAdvancements().getOrStartProgress(player.getServer().getAdvancements().getAdvancement(WealdBladeGesturesTrigger.ID)).isDone())
			return;
