...
public class BibbitArmorLayer<T extends Bibbit, M extends BibbitModel<Bibbit>, A extends BibbitModel<Bibbit>> extends RenderLayer<Bibbit, BibbitModel<Bibbit>> {
   private static final Map<String, ResourceLocation> ARMOR_LOCATION_CACHE = Maps.newHashMap();
   private final BibbitArmorModel innerModel;
   private final TextureAtlas armorTrimAtlas;

   public BibbitArmorLayer(RenderLayerParent<Bibbit, BibbitModel<Bibbit>> pRenderer, BibbitArmorModel pInnerModel, ModelManager pModelManager) {
      super(pRenderer);
      this.innerModel = pInnerModel;
      this.armorTrimAtlas = pModelManager.getAtlas(Sheets.ARMOR_TRIMS_SHEET);
   }

   private void renderArmorPiece(PoseStack pPoseStack, MultiBufferSource pBuffer, Bibbit pLivingEntity, EquipmentSlot pSlot, int pPackedLight, BibbitArmorModel pModel) {
      ...
      if ($$9 instanceof ArmorItem armoritem) {
         if (armoritem.getEquipmentSlot() == pSlot) {
            ...
            net.minecraft.client.model.Model model = this.getArmorModel(EquipmentSlot.FEET);
   ...
   protected void setPartVisibility(BibbitArmorModel pModel, EquipmentSlot pSlot) {
      pModel.setAllVisible(false);
       if (Objects.requireNonNull(pSlot) == EquipmentSlot.FEET) {
           pModel.right_leg.visible = true;
           pModel.left_leg.visible = true;
       }
   ...
   private BibbitArmorModel getArmorModel(EquipmentSlot pSlot) {
   ...
   @Override
   public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, Bibbit pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
