...
public class ReplaceItemChanceModifier extends LootModifier {
	...
	protected @NotNull ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
		...
		float finalChance;
		float finalBothChance;

		if (this.bothChance > 0.0f) {

			float multiplier = 1.0f + lootingLevel * (4.0f / 3.0f);
			float total = this.chance + this.bothChance;


			finalBothChance = Math.min(this.bothChance * multiplier, total);

			finalChance = total - finalBothChance;
		} else {

			finalChance = this.chance;
			finalBothChance = 0.0f;
		}
		...
		for (int i = 0; i < generatedLoot.size(); i++) {
			...
			if (stack.is(this.targetItem)) {
				...
				} else if (r < finalChance + finalBothChance) {
